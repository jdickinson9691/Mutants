namespace ChronTravelers.Engine.Content;

// Plain, System.Text.Json-friendly shapes mirroring the JSON files in
// ChronTravelers.Content — docs/AGENTS.md's Content Agent owns that JSON;
...
/// mechanical parameters consumed by ChronTravelers.Engine.Combat.CombatSession
...
public sealed class AbilityData
{
    ...
    /// ChronTravelers.Engine.Combat.AbilityEffectType for what each does.
    /// </summary>
...
// ---------------------------------------------------------------------------
// Continuous-timeline content (ChronTravelers.Core.Time): tier-free catalogs the
...
public sealed class MonsterSpeciesData
{
    ...
    /// <summary>One of ChronTravelers.Core.Time.MonsterArchetype's names: "Baseline", "Caster", "Bruiser", "Skirmisher".</summary>
...
public sealed class ItemArchetypeData
{
    ...
    /// <summary>One of ChronTravelers.Core.Items.ConsumableEffectType's names; defaults to "None".</summary>
    public string Effect { get; set; } = "None";

    public double EffectMagnitude { get; set; }
    public int EffectDurationTicks { get; set; }

    /// <summary>One of ChronTravelers.Core.Items.RangedKind's names ("Wand"/"Bow"/"Gun"); "None" for a non-ranged archetype. When set, <c>type</c> must be "Ranged". Additive — old JSON without it loads as "None".</summary>
    ...
    /// <summary>One of ChronTravelers.Core.Items.RangedEffectType's names ("Weaken"); "None" for a damage-only ranged weapon. <see cref="EffectMagnitude"/> doubles as the ranged damage multiplier / Weaken amount.</summary>
