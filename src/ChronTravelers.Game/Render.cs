...
internal static class Render
{
    ...
    {
        var p = session.Player;
        var ions = p.Ions.Uncapped ? $"{p.Ions.Current} Ions" : $"{p.Ions.Current}/{p.Ions.Max} Ions";
        session.Send($"{p.Name} the {p.Class} — HP {p.Health.Current}/{p.Health.Max}  {ions}  Credits {p.Credits}  Level {p.Level}  Year {p.CurrentYear} A.D.  Furthest {p.FurthestYearReached}  {p.Position}");
    }
