...
internal static class Commands
{
    ...
    private static void Heal(Session session)
    {
        ...
        if (p.Ions.Current <= 0)
        {
            session.Send("Not enough Ions to heal.");
            return;
        }

        var healed = p.Heal();
        session.Send($"You heal for {healed} HP. ({p.Health.Current}/{p.Health.Max} HP, {p.Ions.Current} Ions left)");
    }
    ...
    private static void Convert(Session session, string arg)
    {
        ...
        session.Send($"Converted {item.Name} for {gained} Ions. ({session.Player.Ions.Current} Ions)");
    }
    ...
    private static void Travel(SharedGame game, Session session, string arg)
    {
        ...
        var cost = IonEconomy.TimeTravelCost(p.CurrentYear, target);
        if (!p.Ions.CanAfford(cost))
        {
            session.Send($"Not enough Ions ({cost} needed, you have {p.Ions.Current}).");
            return;
        }
        ...
        session.Send($"You travel to {target} A.D. — {game.World.GetYear(target).Era.Name}. ({cost} Ions)");
        Render.Room(game, session);
    }
