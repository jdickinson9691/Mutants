...
/// <summary>
/// A character's Tachyon pool. Tachyons below 0 are not allowed — docs/GDD.md §2
...
/// <see cref="Add"/> (converting loot, an ability's Tachyon refund) never
/// clamps, so you can stockpile Tachyons past the nominal <see cref="Max"/>
...
/// </summary>
public sealed class TachyonPool
{
    ...
    public TachyonPool(int max, int? current = null, bool uncapped = false)
    {
        if (max < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(max), max, "Max Tachyons cannot be negative.");
        }
    ...
    public void SetMax(int newMax)
    {
        if (newMax < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(newMax), newMax, "Max Tachyons cannot be negative.");
        }
    ...
    /// <summary>
    /// Adds Tachyons, returning the amount actually added. A capped pool — or
    ...
    /// <summary>True if at least <paramref name="amount"/> Tachyons are available to spend.</summary>
    public bool CanAfford(int amount) => amount >= 0 && Current >= amount;

    /// <summary>Spends Tachyons. Throws if the pool cannot afford it — callers should check CanAfford first.</summary>
    public void Spend(int amount)
    {
        if (!CanAfford(amount))
        {
            throw new InvalidOperationException(
                $"Cannot spend {amount} Tachyons with only {Current} available.");
        }
