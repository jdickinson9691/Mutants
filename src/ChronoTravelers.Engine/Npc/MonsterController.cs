...
/// monster heals if hurt (spending Tachyons, first converting a carried item
...
/// (see <see cref="ChronoTravelers.Core.Monsters.AggroModel"/>): stepping onto its
...
public static class MonsterController
{
    ...
    /// <summary>Hurt monster → heal (converting carried fodder first if out of Tachyons). True if it acted.</summary>
    private static bool TryHeal(Monster monster)
    {
        ...
        if (monster.Tachyons.Current == 0 && monster.Inventory.Count > 0)
        {
    ...
    /// <summary>Fraction of its Tachyon pool below which a monster will scavenge a ground item to burn for fuel.</summary>
    private const double ScavengeForTachyonsBelow = 0.5;
