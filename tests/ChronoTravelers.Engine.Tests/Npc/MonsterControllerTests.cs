...
public class MonsterControllerTests
{
    ...
    [Fact]
    public void Tick_AHurtMonsterWithTachyonsHeals()
    {
        var map = FourRoomMap();
        var pop = EmptyPopulation(map);
        var monster = new Monster("Bleeder", 2, maxHp: 40, attackPower: 8, defense: 3, speed: 10, xpReward: 80, maxTachyons: 30);
        ...
        Assert.True(monster.Health.Current > 10, "A hurt monster with Tachyons should heal.");
        Assert.True(monster.Tachyons.Current < 30, "Healing should spend Tachyons.");
    }
    ...
    public void Tick_AHurtBrokeMonsterConvertsACarriedItemThenHeals()
    {
        var map = FourRoomMap();
        var pop = EmptyPopulation(map);
        var monster = new Monster("Scrounger", 2, maxHp: 40, attackPower: 8, defense: 3, speed: 10, xpReward: 80, maxTachyons: 30);
        ...
        Assert.True(monster.Health.Current > 8, "It should convert the scrap for Tachyons and heal.");
    }
    ...
    public void Tick_ACalmHealthyMonsterOnAPileOfLootLeavesItAlone()
    {
        var map = FourRoomMap();
        var pop = EmptyPopulation(map);
        var monster = Monster.Create("Rover", tier: 1); // full HP, full Tachyons
        monster.PlaceAt(Coordinate.Origin);
        pop.AddMonster(monster);
    ...
    [Fact]
    public void Tick_ALowTachyonMonsterScavengesOneNonWeaponFromThePileToBurn()
    {
        var map = FourRoomMap();
        var pop = EmptyPopulation(map);
        var monster = new Monster("Scrounger", 2, 40, 8, 3, 10, 80, maxTachyons: 30);
        monster.Tachyons.Spend(monster.Tachyons.Current); // bone dry -> below the scavenge threshold
        monster.PlaceAt(Coordinate.Origin);
        pop.AddMonster(monster);
    ...
    public void Tick_AMonsterNeverTakesATimeShard_EvenBrokeAndUnarmed()
    {
        ...
        var pop = EmptyPopulation(map);
        var monster = new Monster("Scrounger", 2, 40, 8, 3, 10, 80, maxTachyons: 30);
        monster.Tachyons.Spend(monster.Tachyons.Current); // broke -> would scavenge fuel
        monster.PlaceAt(Coordinate.Origin);
        pop.AddMonster(monster);
