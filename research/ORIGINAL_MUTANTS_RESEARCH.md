...
- [MBBSEmu GitHub issue #94 — time travel / ions warning](https://github.com/enusbaum/MBBSEmu/issues/94)
...
## 4. Core survival resource: Ions

- The game's core resource is called **ions**, described as *"the equivalent to
  food."* Required to survive and to heal wounds.
- Ions are generated by **converting** any item in the game: `convert skull`
  is the example command given, which destroys the item in a flash of light and
  raises the player's ion level.
- A modern hobbyist forum post (2020, Mutants V4 on WG3) uses the word
  **"energy"** interchangeably with ions for the same converted-item resource,
...
- A MBBSEmu GitHub issue (#94) is titled *"printf Warning when attempting to
  time travel without enough ions,"* which independently confirms that **ions
  are the currency spent to time-travel between levels**, not just a
  food/healing resource.
- Ions are also stated to power spellcasting and "other special characteristics
  of the player" (per the Everything2 summary) — i.e., ions are a unified
...
- Items found or looted can be: **wielded** as a weapon, **sold to a store**
  for Riblets, or **converted** to ions.
...
- **Time travel** is the mechanic that moves the player between distinct
  ...
  different content, difficulty, and loot tables). Time travel costs ions
  (confirmed by MBBSEmu issue #94); no source specifies the exact ion cost
